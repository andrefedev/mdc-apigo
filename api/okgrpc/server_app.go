...
func (s Server) DeliveryDayListAll(ctx context.Context, req *v1.DeliveryDayListAllReq) (*v1.DeliveryDayListAllRes, error) {
	...
	filter := app.NewDeliveryDayFilterInput(req.GetFilter())
	if err := filter.Validate(); err != nil {
		return nil, err
	}

	paging := app.NewDeliveryDayPagingInput(req.GetPaging())
	if err := paging.Validate(); err != nil {
		return nil, err
	}

	result, err := s.useservice.DeliveryDayListAll(ctx, filter, paging)
	if err != nil {
		return nil, err
	}
